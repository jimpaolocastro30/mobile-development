...
const avatarBaseUrl = 'http://34.123.170.125:3006/images/avatars/'
// const apiBaseUrl = process.env.REACT_NATIVE_AVATAR_URL
const imageDir = FileSystem.documentDirectory + 'avatars/'
...
const fetchImagesFromServer = async (avatars) => {
	try {
		await ensureDirExists()
		console.log("fetchImageFromServer")
		console.log('Downloading', avatars.length, 'image files...')
		await Promise.all(avatars.map(async ({avatar_id, image_path}) => {
